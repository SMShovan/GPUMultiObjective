/**
 * @file parallelStressTest.cpp
 * @brief Randomized stress test for the parallel (OpenMP) SOSP update
 * algorithm.
 *
 * Generates random graphs with random changes, runs both full Dijkstra
 * (ground truth) and the parallel SOSP update algorithm, and compares
 * distances. Uses random seeds for each iteration to test robustness.
 *
 * Identical to stressTest.cpp except it calls parallelSOSPUpdate() instead
 * of sequentialSOSPUpdate().
 */
...
int main() {
  ...
  for (int run = 0; run < totalRuns; ++run) {
    ...
    ok = ok &&
         generateGraphCSR(numberOfNodes, numberOfEdges, true, originalPrefix,
                          numberOfObjectives, 1, objectiveEndRange, graphSeed);

    ok = ok && generateChangedEdges(
                   1, objectiveEndRange, numberOfObjectives, numberOfNodes,
                   numberOfChangedEdges, insertPct, deletePct, true, true, true,
                   false, originalPrefix, insertPath, deletePath, changeSeed);

    ok = ok && updateGraphCSR(originalPrefix, updatedPrefix, insertPath,
                              deletePath, true);
    ...
    ok = ok && parallelSOSPUpdate(originalPrefix,
                                  expectedDir + "/distancesOriginal.txt",
                                  expectedDir + "/SSSPTreeOriginal.txt",
                                  insertPath, deletePath, objectiveIndex, 0,
    ...
    if (match) {
      ...
      cout << "Run " << run << ": FAIL (nodes=" << numberOfNodes
           << " edges=" << numberOfEdges << " objs=" << numberOfObjectives
  ...
  cout << "\n=== Parallel Stress Test: " << passCount << "/" << totalRuns
